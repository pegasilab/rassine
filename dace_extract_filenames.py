...
import numpy as np
import pandas as pd
from typing import Optional, List, Dict, Set, Iterable, Sequence, Tuple, Mapping
...
def get_parser() -> ParametersParser:
    ...
    """
    res = ParametersParser(strict_config_sections=['dace-extract-filenames'],
                           description="""\
    ...
    """)
    res.add_config_argument('--config-file', '-c', type=Path, action='append', env_var_name='RASSINE_CONFIG_FILES',
                            help='Use the specified configuration files. Files can be separated by commas/the command can be invoked multiple times.')
    res.add_argument('--dace-input-file', type=Path,
                     help='DACE CSV input file path')
    res.add_argument('--root-path', env_var_name='RASSINE_ROOT_PATH', type=Path,
                     help='Root path of the data')
    res.add_argument('--input-data-path', type=Path,
                     help='Relative path to the input data files')
    return res
