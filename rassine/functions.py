...
def intersect_all_continuum_sphinx(
    ...
    if nthreads >= multicpu.cpu_count():
        ...
        if (
            nthreads == 1
        ):  # was os.getcwd()=='/Users/cretignier/Documents/Python': #to restablish Rassine on my computer only
            product = [
...
def preprocess_match_stellar_frame(
    files_to_process, args=None, rv=0, dlambda=None, final_sound=True
):
    """
    ...
    for name in files_to_process:
        ...
        mask = (wave >= (wave_min_k - dlambda / 2.0)) & (wave <= (wave_max_k + dlambda / 2.0))
        mask2 = (wave >= (hole_left_k - dlambda / 2.0)) & (wave <= (hole_right_k + dlambda / 2.0))
        nflux[mask2] = 0
        new_flux = nflux[mask]
        nflux_err[mask2] = 1
        ...
        save_pickle(
            name,
            {
                ...
                "mjd": f["mjd"],
                "jdb": f["jdb"],
                "hole_left": hole_left_k,
                "hole_right": hole_right_k,
                "wave_min": wave_min_k,
                "wave_max": wave_max_k,
                "dwave": dlambda,
            },
...
def preprocess_stack(files_to_process, bin_length=1, dbin=0, make_master=True):
    ...
    for j in group:
        ...
        save_pickle(
            directory + "/Stacked_spectrum_bin_" + str(bin_length) + "." + date_name + ".p", out
        )
...
def smooth(
    y: npt.ArrayLike, box_pts: int, shape: str = "rectangular"
):  # rectangular kernel for the smoothing
