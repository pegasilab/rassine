...
from rassine.arguments import argument_parser

config = vars(argument_parser(cwd).parse_args())
...
count_cut_lim = config["count_cut_lim"]
count_out_lim = config["count_out_lim"]
denoising_dist = config["denoising_dist"]
feedback = config["feedback"]
float_precision = config["float_precision"]
interpol = config["interpol"]
light_version = config["light_version"]
mask_telluric = config["mask_telluric"]
only_print_end = config["only_print_end"]
output_dir = config["output_dir"]
outputs_denoising_saved = config["outputs_denoising_saved"]
outputs_interpolation_saved = config["outputs_interpolation_saved"]
par_R = config["par_R"]
par_Rmax = config["par_Rmax"]
par_fwhm = config["par_fwhm"]
par_reg_nu = config["par_reg_nu"]
par_smoothing_box = config["par_smoothing_box"]
par_smoothing_kernel = config["par_smoothing_kernel"]
par_stretching = config["par_stretching"]
par_vicinity = config["par_vicinity"]
...
if spectrum_name.split(".")[-1] == "fits":  # to load a fits file
    ...
    if spectrum_name.split(".")[-1] == "csv":
        ...
        if np.shape(data)[1] != 2:
            print(
                "[WARNING] Your txt file does not contain two columns, please take care to format correctly your input"
            )
        if (~(data[0][0] == data[0][0])) & ((data[1][0] == data[1][0])):
            print(
                "[WARNING] Your txt file is suspected to contain a header, the first line was removed (please take care to format correctly your txt input)"
            )
            data = data[1:, :]
...
if par_fwhm == "auto":
    ...
    continuum_left = np.ravel(pd.DataFrame(spectre[::-1]).rolling(int(30 / dgrid)).quantile(1))[
        ::-1
    ]
    ...
    if errors_fit[-1] / popt[-1] > 0.2:
        print(
            " [WARNING] Error on the FWHM of the CCF > 20% ! Check the CCF and/or enter you own mask"
        )
...
if (feedback) & (par_smoothing_box != "auto"):
    ...
    (l1,) = plt.plot(
        grid,
        ras.smooth(spectre, int(par_smoothing_box), shape=par_smoothing_kernel),
        color="k",
    ...
    if not only_print_end:
        print(
            " Smoothed kernel length saved at : %s with %s shape"
            % (ssmoothing.val, callback.shape)
        )
    plt.close()
    ...
    if par_smoothing_box == "auto":
        if par_smoothing_kernel not in ["erf", "hat_exp"]:
            if not only_print_end:
                print(
                    " [WARNING] Your smoothing kernel is not correctly specified, pleaser enter either : erf or hat_exp"
                )
...
if (par_Rmax != par_R) | (par_Rmax == "auto"):
    ...
    continuum_small_win = np.ravel(
        pd.DataFrame(spectre[subset]).rolling(int(windows * dx / speedup), center=True).quantile(1)
    ...
    continuum_left = np.ravel(
        pd.DataFrame(spectre[subset][::-1]).rolling(int(big_windows * dx / speedup)).quantile(1)
    )[::-1]
    continuum_right[np.isnan(continuum_right)] = continuum_right[~np.isnan(continuum_right)][0]
    continuum_left[np.isnan(continuum_left)] = continuum_left[~np.isnan(continuum_left)][-1]
    both = np.array([continuum_right, continuum_left])
    ...
    for j in range(
        ...
        continuum_right[np.isnan(continuum_right)] = continuum_right[~np.isnan(continuum_right)][
            0
        ...
        while (loop) & (threshold > 0.2):
            ...
            for j in range(len(cluster_length)):
                cluster_length[j, 4] = np.nanpercentile(
                    abs(np.diff(spectre[int(cluster_length[j, 0]) : int(cluster_length[j, 1])])),
                    10,
                )
    ...
    if feedback:
        ...
        radius = (
            grid
            / minx
            * (par_R + (par_Rmax - par_R) * penalite_step ** (float(reg.split("_")[-1])))
        )
        ...
        radius2 = (
            ...
            * (
                par_R
                + (par_Rmax - par_R)
                * (1 + np.exp(-10 * float(reg.split("_")[-1]) * (penalite_step - ini))) ** -1
            )
        )
        law2 = (
            par_R
            + (par_Rmax - par_R) * (1 + np.exp(-10 * float(reg.split("_")[-1]) * (t - ini))) ** -1
        )
        ...
        plt.ylabel("Flux [arb. unit]", fontsize=13)
        ax = plt.gca()
        ...
        class Index:
            ...
            def update(self, val):
                ...
                if self.model == "poly":
                    ax.set_ylim(
                        par_R - (par_Rmax - par_R) * 0.1 - 1,
                        par_Rmax + (par_Rmax - par_R) * 0.1 + 1,
                    )
                    ax2.set_ylim(
                        par_R - (par_Rmax - par_R) * 0.1 - 1,
                        par_Rmax + (par_Rmax - par_R) * 0.1 + 1,
                    ...
                    radius2 = (
                        ...
                        * (
                            par_R
                            + (par_Rmax - par_R)
                            * (1 + np.exp(-10 * expo * (penalite_step - expo2))) ** -1
                        )
                    )
                    l3.set_ydata(radius2)
                    l4.set_ydata(
                        par_R + (par_Rmax - par_R) * (1 + np.exp(-10 * expo * (t - expo2))) ** -1
                    )
                fig.canvas.draw_idle()
        ...
        if callback.model == "poly":
            ...
            if not only_print_end:
                print(
                    " You selected the poly law (exponent %.2f) with R : %.2f and Rmax %.2f"
                    % (expo, par_R, par_Rmax)
                )
        else:
            radius = law_chromatic * (
                par_R
                + (par_Rmax - par_R) * (1 + np.exp(-10 * expo * (penalite_graph - expo2))) ** -1
            )
            par_model = "sigmoid_%.2f_%.2f" % (expo, expo2)
            if not only_print_end:
                print(
                    " You selected the sigmoid law (sigma %.2f, center %.2f) with R : %.2f and Rmax %.2f"
                    % (expo, expo2, par_R, par_Rmax)
                )
        plt.close()
    else:
        if reg.split("_")[0] == "poly":
            ...
            radius = law_chromatic * (
                par_R + (par_Rmax - par_R) * (1 + np.exp(-width * (penalite_graph - center))) ** -1
            )
            par_model = reg
        else:
            if not only_print_end:
...
while loop == "y":
    ...
    if feedback:
        ...
        Interpol = interp1d(
            wave[1:-1], flux[1:-1], kind=interpol, bounds_error=False, fill_value="extrapolate"
        )
        ...
        plt.show(block=False)
        loop = ras.sphinx(
            "Do you want to perform a new loop with R = KxR (y/n)? Enter the K that you want (if (y) is given the default value is K=1.5)",
            rep=["y", "n"]
            + ["%.0f" % (j) for j in np.arange(1, 10)]
            + ["%.1f" % (j) for j in np.arange(1, 10, 0.1)],
        )
        plt.close()
        if loop == "n":
            break
...
if feedback:
    ...
    plt.xlim(minx + 9 * len_x / 10.0, None)
    ...
    class Index(object):
        ...
        def update(self, event):
            ...
            flux[0 : self.ind + 1] = flux[self.ind + 1]
            flux[-1 - self.ind :] = flux[-2 - self.ind]
            self.ind += 1
...
if feedback:
    ...
    y1, y2 = np.min(spectre[center - win_grap1 : center + win_grap2]), np.max(
        spectre[center - win_grap1 : center + win_grap2]
    )
    y1, y2 = np.min(flux[sort_val[0] - win_grap1 : sort_val[0] + win_grap2]), np.max(
        flux[sort_val[0] - win_grap1 : sort_val[0] + win_grap2]
    )
    ...
    class Index(object):
        ...
        def update(self, event):
            ...
            l3.set_xdata(grid[center])
            l3.set_ydata(spectre[center])
            lvline.set_xdata(x=grid[center])

            l7.set_xdata(grid[center])
            l7.set_ydata(spectre[center])
            ...
            l2.set_xdata(wave[mask_out])
            l2.set_ydata(flux[mask_out])
            l4.set_ydata(continuum)
        ...
        def backup(self, event):
            if len(self.wave_backup) > 1:
                ...
                Interpol = interp1d(
                    self.wave_backup[-1][~self.out_backup[-1]],
                    self.flux_backup[-1][~self.out_backup[-1]],
                    kind=interpol,
                    bounds_error=False,
                    fill_value="extrapolate",
                )
        ...
        def next_line(self, event):
            if self.ind < lines_showed - 1:
                ...
                lvline.set_xdata(x=grid[center])
                ax.set_xlim(
                    wave[sort_val[self.ind] - win_grap], wave[sort_val[self.ind] + win_grap + 1]
                )
                y1, y2 = np.min(
                    flux[sort_val[self.ind] - win_grap : sort_val[self.ind] + win_grap + 1]
                ), np.max(flux[sort_val[self.ind] - win_grap : sort_val[self.ind] + win_grap + 1])
                ax.set_ylim(y1 - (y2 - y1) * 0.2, y2 + (y2 - y1) * 0.2)
                l7.set_xdata(grid[center])
                l7.set_ydata(spectre[center])
                l3.set_xdata(grid[center])
...
if not only_print_end:
    print(
        " Number of points removed to build a more equidistant grid : %s"
        % (len(wave_backup) - len(wave))
    )
...
if feedback:
    ...
    loop = ras.sphinx(
        "Double click on : 1) blue points to add them 2) black point to remove them. Press Enter to finish the process."
    )
    plt.close()
...
print(
    "Ouput file saved under : %s (SNR at 5500 : %.0f)"
    % (output_dir + "RASSINE_" + new_file + ".p", output["parameters"]["SNR_5500"])
)

if False:
    ras.make_sound(
        "Congratulations ! Racine has finished your spectra normalisation. Time to make science."
    )
