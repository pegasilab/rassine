...
if len(sys.argv) > 1:
    ...
    for j in optlist:
        ...
        if (
            j[0] == "-k"
        ):  # pickle file containing the RV to remove or float number with systemic velocity
            rv = j[1]
            if len(rv.split("/")) == 1:
                rv = np.float(rv)
            else:
                ...
                if rv_file.split(".")[-1] == "csv":
                    rv = pd.read_csv(rv_file)[
                        "model"
                    ]  # RV time-series to remove in kms, (binasry  or known planets) otherwise give the systemic velocity
...
if len(files_to_reduce.split(".csv")) > 1:
    print(" [INFO] Table founded")
    rassine_files_to_preprocess = np.sort(pd.read_csv(files_to_reduce)["fileroot"])
else:
    print(" [INFO] Table not founded")
...
if process == "RASSINE":
    ...
    if not len(rassine_files_to_reduce):
        sys.exit(" [ERROR] No file found to be reduced")
    ...
    if nthreads >= multicpu.cpu_count():
        ...
        if len(rassine_files_to_reduce) < nthreads:
            nthreads = len(rassine_files_to_reduce)
            print("Less spectra than cpu, cpu reduced to : %s" % (nthreads))
    ...
    if not len(rassine_files_to_reduce):
        sys.exit(" [ERROR] No file found to be intersected")
    ...
    if not len(rassine_files_to_reduce):
        sys.exit(" [ERROR] No file found to be savgol scaled")
    ...
    if not len(rassine_files_to_preprocess):
        sys.exit(" [ERROR] No file found to be preprocessed")
    ...
    if nthreads >= multicpu.cpu_count():
        ...
        if len(rassine_files_to_preprocess) < nthreads:
            nthreads = len(rassine_files_to_preprocess)
            print("Less spectra than cpu, cpu reduced to : %s" % (nthreads))
    ...
    if not len(rassine_files_to_reduce):
        sys.exit(" [ERROR] No file found to be match")
