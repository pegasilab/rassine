...
def nonneg_int_or_auto(s: str) -> Union[Literal['auto'], int]:
    ...
    if s == 'auto':
        ...
        raise argparse.ArgumentTypeError("'{}' is not a valid non-negative integer or 'auto'".format(s))
...
def mask_telluric_type(s: str) -> List[Tuple[float, float]]:
    ...
    pair = (string('[') >> seq(number << string(','), number) << string(']')).map(lambda s: (s[0], s[1]))
...
def argument_parser(cwd: str='') -> argparse.ArgumentParser:
    ...
    parser.add_argument('--spectrum_name', '-s', type=str, action=SpectrumAction,
                        default=cwd + '/spectra_library/spectrum_cenB.csv',
                        help='Full path of your spectrum pickle/csv file')

    parser.add_argument('--output_dir', '-o', type=str, default=cwd + '/output/', action=StoreExceptUnspecifiedAction,
    ...
    parser.add_argument('--anchor_file', '-l', type=str, default='',
    ...
    parameters.add_argument('--par_stretching', '-p', type=par_stretching_type,
                            default='auto_0.5',
    ...
    parser.add_argument('--CCF_mask', type=str, default='master',
                        help="Only needed if par_fwhm is in 'auto'")
    parser.add_argument('--RV_sys', type=float, default=0.0,
                        help="RV systemic in kms, only needed if par_fwhm is in 'auto' and CCF different of 'master'")

    parser.add_argument('--mask_telluric', type=mask_telluric_type, default='[[6275,6330],[6470,6577],[6866,8000]]',
                        help="A list of left and right borders to eliminate from the mask of the CCF only if CCF = 'master' and par_fwhm = 'auto'")

    parameters.add_argument('--par_R', '-r', type=float_or_auto, default='auto',
                            help="Minimum radius of the rolling pin in angstrom ('auto' available)")
    parameters.add_argument('--par_Rmax', '-R', type=float_or_auto, default='auto',
                            help="Maximum radius of the rolling pin in angstrom ('auto' available)")
    parameters.add_argument('--par_reg_nu', type=str, default='poly_1.0',
                            help="Penality-radius law, either poly_d (d the degree of the polynome x**d) or or sigmoid_c_s where c is the center and s the steepness")  # TODO why the comma

    parser.add_argument('--denoising_dist', type=int, default=5,
                        help="Half window of the area used to average the number of point around the local max for the continuum")
    parser.add_argument('--count_cut_lim', type=int, default=3,
                        help="Number of border cut in automatic mode (put at least 3 if Automatic mode)")
    parser.add_argument('--count_out_lim', type=int, default=1,
                        help="Number of outliers clipping in automatic mode (put at least 1 if Automatic mode)")
    parser.add_argument('--interpol', type=str, default='cubic',
    ...
    parser.add_argument('--feedback', '-a', type=str2bool, default=True, action=StoreExceptUnspecifiedAction,
                        help="Run the code without graphical feedback and interactions with the sphinx (only wishable if lot of spectra)")
    parser.add_argument('--only_print_end', '-P', type=str2bool, default=False,
                        help="")
    parser.add_argument('--plot_end', '-e', type=str2bool, default=True,
                        help="")
    parser.add_argument('--save_last_plot', '-S', type=str2bool, default=False,
                        help="")
    parser.add_argument('--outputs_interpolation_saved', type=str, default='linear',
                        choices=['linear', 'cubic', 'all'],
                        help="To only save a specific continuum (output files are lighter), either 'linear','cubic' or 'all'")
    parser.add_argument('--outputs_denoising_saved', type=str, default='undenoised',
    ...
    parser.add_argument('--light_version', type=str2bool, default=True,
                        help="To save only the vital output")
