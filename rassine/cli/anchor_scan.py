...
def import_files_mcpu(file_liste, kind):
    ...
    snr = []
    for j in file_liste:
        file = open_pickle(j, RassinePickle)
        snr.append(file["parameters"]["SNR_5500"])
        sub.append(file["output"]["anchor_index"])
    return sub, snr
...
def intersect_all_continuum_sphinx(
    ...
    if nthreads >= multicpu.cpu_count():
        ...
        if (
            ...
            product = pool.map(import_files_mcpu_wrapper, zip(chunks, repeat(None)))
    ...
    if master_spectrum is not None:
        ...
        file: RassinePickle = open_pickle(Path(master_spectrum))
    ...
    plt.plot(
        file_to_plot["wave"],
        file_to_plot["flux"] / file_to_plot["output"]["continuum_linear"],
        color="k",
    )
    ax = plt.gca()
    ...
    for i, j in enumerate(names):
        ...
        plt.scatter(
            file_to_read["output"]["anchor_wave"],
            i * np.ones(len(file_to_read["output"]["anchor_wave"])),
            alpha=0.5,
        )
...
def get_parser():
    ...
    parser.add_argument(
        "--input-directory", type=Path, help="Path to the directory containing the input files"
    )
    parser.add_argument(
        "--master-spectrum",
        type=str,
    ...
    parser.add_argument(
        "--nthreads", type=int, default=1, help="Number of threads for multiprocessing"
    )
    parser.add_argument(
        "--fraction", type=float, default=0.2, help="Parameter of the model between 0 and 1"
    )
    parser.add_argument(
        "--threshold", type=float, default=0.66, help="Parameter of the model between 0 and 1"
    )
    parser.add_argument(
        "--tolerance", type=float, default=0.5, help="Parameter of the model between 0 and 1"
    )
    return parser
...
def cli():
    ...
    intersect_all_continuum_sphinx(
        ...
        copies_master=args.copies_master,
