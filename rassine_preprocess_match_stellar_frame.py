...
def get_parser():
    ...
    res.add_argument('--dlambda', '-d', type=float, default=None,
                     help='Instrument format of the s1d spectra')
    res.add_argument('--no-dlambda', dest='dlambda', action='store_const', const=None,
                     help='Set dlambda to None')
    res.add_argument('--rv', '-k', type=float_or_rv_file, default='0.0',
    ...
    res.add_argument('--output-file', type=Path, required=True,
                     help='Output HDF5 file containing the results')
    return res
...
if __name__ == "__main__":
    ...
    wave_min_k, wave_max_k, dlambda, hole_left_k, hole_right_k, static_grid, wave_min, berv, lamp, plx_mas, acc_sec, rv, rv_mean = preprocess_prematch_stellar_frame(list(map(str, inputfiles)), dlambda=dlambda, rv=rv)
    output = {
